<?php
session_start();
header('Content-Type: application/json; charset=utf-8');

if (!isset($_SESSION['usuario_id'])) {
    http_response_code(401);
    echo json_encode(['ok' => false, 'mensaje' => 'Sesión no activa.']);
    exit;
}

$configPath = dirname(__DIR__, 2) . '/config/voip_config.php';
if (!is_file($configPath)) {
    http_response_code(500);
    echo json_encode(['ok' => false, 'mensaje' => 'Falta config/voip_config.php.']);
    exit;
}

$config = require $configPath;
$accountSid = trim((string)($config['account_sid'] ?? ''));
$authToken = trim((string)($config['auth_token'] ?? ''));

if (!preg_match('/^AC[a-fA-F0-9]{32}$/', $accountSid) || $authToken === '' || $authToken === 'TU_AUTH_TOKEN_PRIVADO') {
    http_response_code(500);
    echo json_encode([
        'ok' => false,
        'mensaje' => 'Configura account_sid y auth_token para consultar el historial.',
    ]);
    exit;
}

function twilioGetJson(string $url, string $accountSid, string $authToken): array
{
    $ch = curl_init($url);
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_FOLLOWLOCATION => true,
        CURLOPT_CONNECTTIMEOUT => 8,
        CURLOPT_TIMEOUT => 20,
        CURLOPT_USERPWD => $accountSid . ':' . $authToken,
        CURLOPT_HTTPAUTH => CURLAUTH_BASIC,
        CURLOPT_HTTPHEADER => ['Accept: application/json'],
    ]);

    $body = curl_exec($ch);
    $status = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
    $error = curl_error($ch);
    curl_close($ch);

    if ($body === false || $status < 200 || $status >= 300) {
        throw new RuntimeException($error !== '' ? $error : 'Twilio respondió HTTP ' . $status);
    }

    $data = json_decode($body, true);
    if (!is_array($data)) {
        throw new RuntimeException('Respuesta JSON no válida de Twilio.');
    }

    return $data;
}

try {
    $base = 'https://api.twilio.com/2010-04-01/Accounts/' . rawurlencode($accountSid);
    $callsData = twilioGetJson($base . '/Calls.json?PageSize=12', $accountSid, $authToken);
    $recordingsData = twilioGetJson($base . '/Recordings.json?PageSize=8', $accountSid, $authToken);

    $calls = [];
    foreach (($callsData['calls'] ?? []) as $call) {
        if (($call['direction'] ?? '') !== 'outbound-dial') {
            continue;
        }

        $calls[] = [
            'sid' => $call['sid'] ?? null,
            'to' => $call['to'] ?? '',
            'from' => $call['from'] ?? '',
            'status' => $call['status'] ?? '',
            'direction' => $call['direction'] ?? '',
            'duration' => isset($call['duration']) ? (int)$call['duration'] : 0,
            'start_time' => $call['start_time'] ?? $call['date_created'] ?? null,
            'end_time' => $call['end_time'] ?? null,
        ];
    }

    $recordings = [];
    foreach (($recordingsData['recordings'] ?? []) as $recording) {
        $sid = (string)($recording['sid'] ?? '');
        if (!preg_match('/^RE[a-fA-F0-9]{32}$/', $sid)) {
            continue;
        }

        $recordings[] = [
            'sid' => $sid,
            'call_sid' => $recording['call_sid'] ?? null,
            'status' => $recording['status'] ?? '',
            'duration' => isset($recording['duration']) ? (int)$recording['duration'] : 0,
            'channels' => isset($recording['channels']) ? (int)$recording['channels'] : null,
            'source' => $recording['source'] ?? '',
            'start_time' => $recording['start_time'] ?? $recording['date_created'] ?? null,
            'audio_url' => 'api/grabacion.php?sid=' . rawurlencode($sid),
        ];
    }

    echo json_encode([
        'ok' => true,
        'calls' => $calls,
        'recordings' => $recordings,
    ], JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
} catch (Throwable $e) {
    http_response_code(502);
    echo json_encode([
        'ok' => false,
        'mensaje' => 'No se pudo consultar el historial de Twilio.',
        'detalle' => $e->getMessage(),
    ], JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
}
